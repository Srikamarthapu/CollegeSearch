...
function ScopedDeadlinePlanner({ scope, colleges, store }: { scope: string; colleges: DeadlineCollege[]; store: PlannerStore }) {
  ...
  async function submitForm(event: FormEvent<HTMLFormElement>) {
    ...
    if (staged.draftPersisted) {
      if (store.discardEditor(scope)) setForm(store.getEditor(scope));
    } else {
      setBusy(true);
      const saved = await store.flush(scope);
      setBusy(false);
      if (saved.persisted && store.discardEditor(scope)) setForm(store.getEditor(scope));
    }
    setErrors({}); setMessage("Task added to your tracker. Review the storage status below for this browser’s saved copy.");
  }
  ...
  async function readBackup(file?: File) {
    ...
    try {
      ...
      setRestore(restored); setMessage("");
  ...
  function taskCard(entry: DeadlineEntry) {
    return <li className={styles.card} key={entry.id}>
      <div className={styles.cardMain}>
        <span className={styles.college}>{colleges.find((college) => college.unitId === entry.collegeId)?.name ?? "College outside this directory"}</span>
      ...
      <div className={styles.cardActions}>
        <label><input type="checkbox" checked={entry.completed} disabled={busy} onChange={(event) => updatePlan({ version: 1, entries: store.get(scope).draft.entries.map((item) => item.id === entry.id ? { ...item, completed: event.target.checked } : item) })} /><span>Completed</span></label>
        <button type="button" disabled={busy || Boolean(form.editor)} onClick={() => openForm(editorForDeadline(entry))}>Edit<span className={styles.srOnly}> {deadlineTitle(entry)}</span></button>
        {removeId === entry.id ? <div className={styles.removeConfirm}><span>Remove this task?</span><button type="button" onClick={() => { updatePlan({ version: 1, entries: store.get(scope).draft.entries.filter((item) => item.id !== entry.id) }); setRemoveId(""); }}>Yes, remove</button><button type="button" onClick={() => setRemoveId("")}>Keep task</button></div> : <button type="button" disabled={busy} onClick={() => setRemoveId(entry.id)}>Remove<span className={styles.srOnly}> {deadlineTitle(entry)}</span></button>}
      </div>
    </li>;
  }
  ...
  return <section className={styles.planner} aria-labelledby={`${id}-heading`}>
    <header className={styles.header}>
      <div><span className={styles.eyebrow}><CalendarDays size={16} aria-hidden="true" />Your next steps</span><h2 id={`${id}-heading`}>Dates you want to keep in view.</h2><p>Add dates from the college’s official pages or your own plan. CollegeSearch does not supply or verify deadlines, cycles, cut-off times, or time zones.</p></div>
      <button className={styles.primary} type="button" disabled={Boolean(form.editor) || snapshot.draft.entries.length >= DEADLINE_LIMIT || busy} onClick={() => openForm(emptyDeadlineEditor())}><Plus size={17} aria-hidden="true" />Add a task</button>
    </header>
    ...
    {form.editor ? <form ref={formRef} className={styles.form} onSubmit={submitForm} noValidate aria-label={form.editor.editingId ? "Edit a tracked date" : "Add a tracked date"}>
      ...
      {Object.keys(errors).length ? <div className={styles.errors} role="alert"><p>Review these details:</p><ul>{Object.values(errors).map((error) => <li key={error}>{error}</li>)}</ul></div> : null}
      ...
      <div className={styles.actions}><button type="submit" className={styles.primary} disabled={busy}>{busy ? "Saving…" : form.editor.editingId ? "Update task" : "Add to tracker"}</button><button type="button" disabled={busy} onClick={() => { if (store.discardEditor(scope)) { setForm(store.getEditor(scope)); setErrors({}); } else setMessage("This browser could not remove the form draft. It is still available."); }}>Discard form</button>{errors.form && form.editor.editingId ? <button type="button" onClick={() => patchForm({ editingId: "", baseEntry: "" })}>Keep form as a new task</button> : null}</div>
    </form> : null}
    ...
    {snapshot.draft.entries.length === 0 ? <div className={styles.empty}>Start with one date you want to remember. It can be an application step, an aid form, a recommendation request, or a visit.</div> : <div className={styles.groups}>
      ...
      {groups.completed.length ? <details className={styles.completed}><summary>Completed tasks · {groups.completed.length}</summary><ul className={styles.cards}>{groups.completed.map(taskCard)}</ul></details> : null}
    </div>}
    ...
    <footer className={styles.footer}>
      ...
      <div className={styles.actions}><button type="button" onClick={downloadBackup}><Download size={16} aria-hidden="true" />Download backup</button><button type="button" onClick={() => fileRef.current?.click()}><Upload size={16} aria-hidden="true" />Restore backup</button><input ref={fileRef} className={styles.fileInput} type="file" accept=".json,application/json" aria-label="Choose a CollegeSearch deadline backup" onChange={(event) => { void readBackup(event.target.files?.[0]); event.target.value = ""; }} /></div>
    </footer>
    {restore ? <div className={styles.restore}>
      <h3>Review the backup before restoring</h3><p>This replaces this {scope === "guest" ? "guest" : "account"} tracker’s {snapshot.draft.entries.length} current tasks with the {restore.entries.length} tasks below. Download your current backup first if you want to keep both. An unfinished form remains separate.</p>
      <ul>{restore.entries.map((entry) => <li key={entry.id}><strong>{colleges.find((college) => college.unitId === entry.collegeId)?.name}</strong> — {deadlineTitle(entry)} · {formatCalendarDate(entry.date)} · {entry.completed ? "completed" : "open"} · {entry.sourceChecked ? "checked by student" : "source not checked"}</li>)}</ul>
      <div className={styles.actions}><button type="button" className={styles.primary} onClick={() => { updatePlan(restore, true); setRestore(null); setMessage("Backup applied to your list draft. Check the browser storage status for confirmation that it saved."); }}>Replace tracker with this backup</button><button type="button" onClick={() => setRestore(null)}>Keep current tracker</button></div>
    </div> : null}
