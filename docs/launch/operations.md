...
1. Before class, retain a tested release bundle/version and its matching static assets. Record the exact host control/command that restores it, the operator who can use that control, the configuration revision and the expected URL/audience. This review did not use Sites controls or establish their rollback interface; discover the real host's supported operation before claiming a rollback is available.
...
- Capture exact source/artifact/deployment/configuration IDs and a tested, compatible rollback target. Verify the actual host rollback control; no rollback was exercised in this review.
...
- Confirm default-branch CI protection and that scheduled workflows actually run; repository YAML alone does not establish host settings or notification delivery.
...
- Root reports local-production HTTP `/api/account` plus hosted-database integration passed in `hosted-endpoint-integration.json`. Repeat the **deployed website** endpoint/browser deletion flow after publication; local route + hosted DB is not the hosted Worker endpoint. Verify server-only secret presence at runtime without printing it or bundling it into browser assets.
- Verify actual backup availability and restore drill, and the app's browser export/restore. Finish the exact retention/deletion operator process before unsupervised account use.
