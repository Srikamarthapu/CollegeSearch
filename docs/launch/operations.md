...
The Monday artifact-verification workflow on the review branch is configured to preserve failures and summaries; it has not reached the default branch, so an active schedule and delivered alerts are not established. A green run or uploaded artifact is not alert-delivery evidence. Assign the primary and backup recipient, enable the intended failure notifications, and deliberately fail one harmless probe in a non-production/manual workflow. Record who actually received the message, on what channel, delivery time, acknowledgement, and the linked run/report. Also simulate a missing scheduled result: the owner must detect that no completed artifact check exists within eight days. Do not expose a webhook secret or post student data into an alert.
...
1. Before class, retain a tested release bundle/version and its matching static assets. Record the exact host control/command that restores it, the operator who can use that control, the configuration revision and the expected URL/audience. The private Sites deployment control restored version 2 and returned to version 3 during the recorded historical drill. Current version 7 compatibility and the intended rollback target must be identified before a school session. See `artifacts/launch-2026-09-13/rollback-exercise.json` for exact versions and health checks.
...
5. Record elapsed restoration time and what failed. Proposed pilot recovery target is 15 minutes to a functioning known-good student research surface. The historical application drill does not establish a timed school operational response or database recovery target.
...
- Retain the passing dated local and private-hosted probe reports. Recheck the final intended hosted release as justified by changes, then complete the school-network/device rehearsal. The historical 120-request probe excludes assets/rendering and account writes.
...
- Verify custom SMTP sender/DNS/rate limits and actual confirmation/recovery delivery to representative school and external inboxes; retain the completed natural-renewal and two-tab revocation evidence for the private origin; cover representative school devices and network conditions. Verify Google OAuth only if enabled; otherwise keep the provider hidden.
