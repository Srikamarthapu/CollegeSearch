...
- Configure the intended main protection and required CI check, then land the reviewed workflows through the release process. September 13 inspection confirmed main was unprotected with no effective rulesets; weekly verification existed only on the review branch, with no retained scheduled run. Observe actual default-branch scheduling and recipient delivery; see recovery-audit/github-release-controls.md.
