...
|---|---|---|
| Release checks | Final build, types and lint pass; 306/306 regression checks; zero dependency advisories including development dependencies. | Private version 4 is bound to source 450f4b2df8ba4d0f9029fea007c6cde009e5be3c; matching GitHub CI and targeted hosted navigation/planner checks passed. Earlier comprehensive hosted evidence remains dated to its source. |
...
| Responsive accessibility | Mobile navigation, Escape/focus return, 320px planner/profile, and long-name signed-in account layouts checked. | Short landscape menu clipping, nested wheel scrolling, planner/profile focus and backup-review navigation were repaired and exercised; see flow-audit/REPORT.md. Actual enlarged text, reduced motion and additional browser/device coverage remain before broad rollout. |
...
| Publication | Updated version 4 deployed privately with environment revision 1. An unauthenticated request returned 401 and no app content. GitHub branch CI passed for the matching source. | Final image fallback correction passed hosted checks, including rejection of external image URLs and invalid widths. Preserve private access until the release gates are satisfied. |
...
Current release provenance is in `artifacts/launch-2026-09-13/flow-audit/deployment.json`. The previous release record and its comprehensive hosted checks remain in `deployment.json` and `private-hosted-final-smoke.json` at the parent evidence directory. Subsequent evidence-only documentation commits do not change the deployed application source named in that record.
